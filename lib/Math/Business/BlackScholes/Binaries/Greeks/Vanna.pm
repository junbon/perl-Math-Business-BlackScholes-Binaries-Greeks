...
sub onetouch {
    ...
    $subpart_1_1 = pnorm(-$eta * $e) * $A * (-$vol - ($theta_ + $v_) * log($U / $S));
...
=head2 xw_common_function_pelsser_1997

xw_common_function_pelsser_1997
...
sub xw_common_function_pelsser_1997 {
    ...
    for (my $k = 1; $k < $iterations_required; $k++) {
        ...
        if ($k == 1 and (not(abs(2 * $vol * $phi / $S) < $stability_constant))) {
            die
