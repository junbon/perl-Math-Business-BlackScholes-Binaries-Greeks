...
sub put {
    ...
    my $d1   = (log($S / $D) + ($mu + $vol * $vol / 2.0) * $t) / ($vol * sqrt($t));
    my $d2   = $d1 - $vol * sqrt($t);
    my $vega = exp(-$r_q * $t) * dgauss($d2) * $d1 / $vol;
    return $vega;
}
...
sub onetouch {
    ...
    my $part1 = pnorm(-$eta * $e) * $A * log($U / $S) - $eta * dgauss($e) * $pa_e;
    my $part2 = pnorm($eta * $e_) * $A_ * log($U / $S) + $eta * dgauss($e_) * $pa_e_;
    my $vega  = (($U / $S)**(($theta_ + $v_) / $vol)) * $part1 + (($U / $S)**(($theta_ - $v_) / $vol)) * $part2;
...
sub w_common_function_pelsser_1997 {
    ...
    for (my $k = 1; $k < $iterations_required; $k++) {
        ...
        my $phi = (1.0 / ($h * $h)) * ($omega * $dbetak_domega + $beta_k) * $k;
        ...
        #
        if ($k == 1 and (not(abs(2 * $vol * $phi) < $stability_constant))) {
            die
