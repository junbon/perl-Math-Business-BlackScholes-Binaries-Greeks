...
Gets the delta for different options, Vanilla and Foreign for all our bet types
...
sub onetouch {
    ...
    my $part1 = ($theta_ + $v_) * pnorm(-$eta * $e) + $eta * dgauss($e) / $sqrt_t;
    my $part2 = ($theta_ - $v_) * pnorm($eta * $e_) + $eta * dgauss($e_) / $sqrt_t;
...
sub x_common_function_pelsser_1997 {
    ...
    my $mu_dash = sqrt(($mu_new * $mu_new) + (2 * $vol * $vol * $r_q * (1 - $w)));

    my $series_part = 0;
    my $hyp_part    = 0;

    my $stability_constant = Math::Business::BlackScholes::Binaries::get_stability_constant_pelsser_1997($S, $U, $D, $t, $r_q, $mu, $vol, $w, $eta, 2);
